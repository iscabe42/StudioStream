    ...
    <div class="row">
        <div class="col-lg-4">
            <div class="btn-panel btn-panel-conversation">
                <a href="javascript:webCam()" style='color:#6D6D6D;' class="encabezadochat btn  col-lg-6 send-message-btn " role="button"><i class="fa fa-video"></i> Web Cam</a>
                <span onclick="espectadores()" class="encabezadochat btn  col-lg-6  send-message-btn pull-right" role="button"><i class="fas fa-users"></i>Espectadores</span>
				
            </div>
        </div>

        <div class=" col-lg-8">
			<div class="btn-panel btn-panel-msg"><label for=""><span style="color:#6D6D6D;"><strong><mark>Publico</mark></strong></span> &nbsp <strong><span id="mynameuser" style="color:#C50D00;"></span><span id="mynameuserPrivate" style="color:#FF9901; display:none;"></span></strong></label>
                <a href="javascript:configuracionChat();" style='color:#6D6D6D; font-size:11px;' class="encabezadochat btn  col-lg-3  send-message-btn pull-right" role="button"><i class="fas fa-cogs"></i> Configuración</a>
            </div>
        </div>
    </div>
	...
	background:#D1D1D1;
        ...
        <div class="conversation-wrap col-lg-4">
			...
			<div class="mediabox" style=display:none;"">
				<center class="switchCh" id="centerInbox" onclick="switchchat()"><span style=" color:black;">Inbox</span> &nbsp <i style="font-size:10px;"  class="far fa-user"></i>&nbsp<span id="contadorPrivados">0</span> &nbsp <i style="font-size:10px;"  class="far fa-envelope"></i> &nbsp <span id="contadorMSJPrivados">0</span></center>
				<center class="switchCh" id="centerPublico" onclick="switchchat()" style="display:none;"><span style="color:black;">Chat Amigos</span>&nbsp <i style="font-size:10px;" class="far fa-user"></i></center>
			</div>
		...
		<img onclick="limpiarimg()" style="display:none; width:100%; position:relative; z-index:3; cursor:pointer;" src="" id="verimagenSeleccionada" />
		<div class="cuerpoChat"></div>
				<div id="modalconfig" class="modalconfiguracion" style="cursor:pointer; opacity:0.5; display:none; width:94%; height:300px; background:gray; z-index:2; position:absolute;">
				</div>
				<div class="modalconfiguracion" style="margin-top:10px; background:white; display:none; width:50%; height:; z-index:15; position:absolute; left:25%;"> 
					<i onclick="configuracionChat()" style="cursor:pointer; float:right; margin-top:5px; margin-right:10px; color:#9A9A9A;" class="fas fa-times"></i><br>
					<div style="width:100%; background:#EEEEEE; height:1px; color:#EEEEEE;"></div>
						...
						<br>
						<br>
						<button onclick="configuracionChat()" type="button" class="btn" style="background:#DDDDDD; color:black;"><i class="fas fa-check"></i></button>
			...
			function triggerMouseEvent (node, eventType) {
				...
				function bold(){
					if(boldVal==1){
						fontWeight="normal";
						$("#textoModelo").attr("style","color:#"+$("#colorInput").val()+"; font-weight:"+fontWeight+"; text-decoration:"+subr+";");
						$("#spanNegritas").attr("style","font-weight:"+fontWeight+";");
						$("#idinputTextChat").attr("style","color:#"+$("#colorInput").val()+";font-weight:"+fontWeight+";  text-decoration:"+subr+";");
				...
				$(".jscolor").on("change",function (){
					...
					$("#colortrigger").attr("style","margin-left:5px; float:left; position:;  cursor:pointer; border: 1px solid #9A9A9A; border-radius: 4px; width:20px; text-align:center; background:#"+$("#colorInput").val()+"; color:#"+$("#colorInput").val()+";");
            ...
            <div class="send-wrap ">
                ...
                <textarea id="idinputTextChat" class="inputTextChat form-control send-message" rows="3" placeholder="Escriba su mensaje..."></textarea>

			</div>
            <div class="btn-panel">
                <a href="javascript:subirImagen();" style='display:none; color:#6D6D6D; font-size:11px;'  class="encabezadochat col-lg-3 btn   send-message-btn " role="button"><i class="fas fa-cloud-upload-alt"></i> Agregar Imagen</a>
                <input id="file" onchange="prepararImagen()" type="file" class="upfile" style="display:none;"/>
				<a href="javascript:textChat_send();" style='color:#6D6D6D; font-size:11px;'  class="encabezadochat col-lg-4 text-right btn   send-message-btn pull-right" role="button"><i class="fa fa-plus"></i> Enviar mensaje</a>
            </div>
        </div>
    </div>
</div>
		...
		<script type="text/javascript">
			...
			function textChat_send(){
				...
				if((document.getElementById("mynameuserPrivate").style.display)=="inline"){
					if(($(".inputTextChat").val())!=0){
						webSocket.emit('chat_Privado',"sala0","<span  class='pull-left' id='textoAenviar' style='font-weight:"+fontWeight+"; color:"+colorletraEnviar+";  text-decoration:"+subr+";'>"+$(".inputTextChat").val()+"</span>",miNickName,misSalasPrivadas[idmiDestinatarioPrivado][1],miSexo,horaCtual);
						$(".inputTextChat").val("");
					}
				}else{
					if(($(".inputTextChat").val())!=0){
						console.log("enviando a sala---"+miNickName);
						console.log("<span  class='pull-left' id='textoAenviar' style='font-weight:"+fontWeight+"; color:"+colorletraEnviar+";  text-decoration:"+subr+";'>"+$(".inputTextChat").val()+"</span>");
                        
						webSocket.emit('chat_publico',miNickName,"<span class='pull-left' id='textoAenviar' style='font-weight:"+fontWeight+"; color:"+colorletraEnviar+";  text-decoration:"+subr+";'>"+$(".inputTextChat").val()+"</span>",miNickName,miSexo,horaCtual);
						$(".inputTextChat").val("");
					}
			...
			function enviarImagen(){
				...
				if((document.getElementById("mynameuserPrivate").style.display)=="inline"){
					...
					webSocket.emit('chat_Privado',"sala0","<img id='imagenesChat' onclick='tamanioimagen(this)' class='imagenChat' style='width:50px; cursor:pointer;' src='"+imagenUpload+"' />",miNickName,misSalasPrivadas[idmiDestinatarioPrivado][1],miSexo,horaCtual);
					setTimeout(function(){
						$(".inputTextChat").val("");
						},3000);
				}else{
					$(".inputTextChat").val("Enviando imagen...");
					webSocket.emit('chat_publico',"sala0","<img id='imagenesChat' onclick='tamanioimagen(this)' class='imagenChat' style='width:50px; cursor:pointer;' src='"+imagenUpload+"' />",miNickName,miSexo,horaCtual);
					setTimeout(function(){
						$(".inputTextChat").val("");
						},3000);
				}
				...
				if(estatus===1){
					for(var esc=0;esc<listaUsuariosLocal.length;esc++){
						...
						if((listaUsuariosLocal[esc][1])===nickname){
							listaUsuariosLocal[esc][1]=nickname+"...... <i class='fa fa-pencil-alt' aria-hidden='true' style='color:green'></i>";
			...
			for(var escr=0;escr<listaUsuariosLocal.length;escr++){
						...
						if((listaUsuariosLocal[escr][1])===(nickname+"...... <i class='fa fa-pencil-alt' aria-hidden='true' style='color:green'></i>")){
				...
				for(var a=0;a<usuarios_con.length;a++){
					...
					var con_usr=document.getElementById("contenedor_user").innerHTML;
						document.getElementById("contenedor_user").innerHTML=
							...
							"&nbsp <i style=\"font-size:12px; color:"+colorsex+";  margin-top:5px;\" class=\"fa fa-user\" aria-hidden=\"true\"></i>&nbsp"+
			...
			function iniciarPrivado(id){
				...
				if(miNickName!=listaUsuariosLocal[id][1]){
					...
					document.getElementById("contadorMSJPrivados").innerHTML=contadorMensajesPrivados;
						...
						document.getElementById("mynameuserPrivate").innerHTML="&nbsp con: &nbsp"+listaUsuariosLocal[id][1];
						document.getElementById("mynameuserPrivate").style.display="inline";
						misSalasPrivadas.push([miNickName+listaUsuariosLocal[id][1],listaUsuariosLocal[id][1],miSexo,horaCtual,0]);
				...
				function existeEnmiLista(){
					for(var a=0;a<misSalasPrivadas.length; a++){
						if(((miNickName+listaUsuariosLocal[id][1])==misSalasPrivadas[a][0])||((listaUsuariosLocal[id][1]+miNickName)==misSalasPrivadas[a][0])){
							return a+1;
						}
					}
					return false;
			...
			webSocket.on('chat_rpPrivado',function(remitente,destinatario,sexo,horaMensaje){
				...
				function buscaExitSalaP(){
							for(var a=0;a<misSalasPrivadas.length; a++){
								if(((remitente+destinatario)==misSalasPrivadas[a][0])||((destinatario+remitente)==misSalasPrivadas[a][0])){
									return a+1;
								}
							}
							return false;
					...
					for(var a=0;a<misSalasPrivadas.length;a++){
						...
						var con_msj_privado=document.getElementById("contenedor_mensaje_privado").innerHTML;
							document.getElementById("contenedor_mensaje_privado").innerHTML=
								"<div onclick=\"verMensajesPrivados("+a+")\" class=\"inboxMedia media conversation\" style=\"margin-top:5px;\"> "+
								"&nbsp <i style=\"font-size:12px; color:"+colorsex+";  margin-top:5px;\" class=\"fa fa-user\" aria-hidden=\"true\"></i>&nbsp"+
								"<div class=\"media-body\">"+
									"<span style='font-size:11px;' class=\"pull-left media-heading\">"+misSalasPrivadas[a][1]+"</span> "+msjcontador+
								"</div>"+
							"</div>"+con_msj_privado; 
					}
				}
					...
					for(var a=0;a<inboxPrivado.length;a++){
						if(misSalasPrivadas[idmiDestinatarioPrivado][0]==inboxPrivado[a][0]){						
							...
							if(inboxPrivado[a][4]==="male"){
								sexUsuarioActivo="<i style=\"font-size:14px; color:blue; margin-top:5px;\" class=\"fa fa-user\" aria-hidden=\"true\"></i>";
							}
							else if(inboxPrivado[a][4]==="female"){
								sexUsuarioActivo="<i style=\"font-size:14px; color:#FC19D8; margin-top:5px;\" class=\"fa fa-user\" aria-hidden=\"true\"></i>";
							}
							...
							"<div class=\"media msg\">"+
								...
								"<div class=\"media-body\">"+
								   " <small class=\"pull-right time\" style=\"color:#ADADAD;\"><i class=\"far fa-clock\"></i> "+inboxPrivado[a][5]+"</small>"+
			...
			function actualizaChatPrivado(mensaje,nickname,destinatario,misexo,horamensaje){
				if(existeEnmiListaPrivada()!=false){
					if(nickname!=miNickName){
						misSalasPrivadas[existeEnmiListaPrivada()-1][4]=((misSalasPrivadas[existeEnmiListaPrivada()-1][4])+1);
					}
					
					inboxPrivado.push([misSalasPrivadas[(existeEnmiListaPrivada()-1)][0],mensaje,nickname,destinatario,misexo,horamensaje]);
					if(nickname!=miNickName){
				...
				function existeEnmiListaPrivada(){
					for(var a=0;a<misSalasPrivadas.length; a++){
						if((nickname+destinatario==misSalasPrivadas[a][0])||((destinatario+nickname)==misSalasPrivadas[a][0])){
							return a+1;
						}
					}
					return false;
			...
			function local_respuesta_chat(mensajes){
				...
				"<div class=\"media1 msg\">"+
                    "<div class=\"media-body\">"+
                       " <small class=\"pull-right time\" style=\"color:#ADADAD;\"><i class=\"far fa-clock\"></i> "+mensajes[a][3]+"</small>"+
			...
			function respuesta_chat(misexo,nickname,mensaje,horamensaje){
				...
				"<div class=\"media1 msg\">"+
                    "<div class=\"media-body\">"+
                       " <small class=\"pull-right time\" style=\"color:#ADADAD;\"><i class=\"far fa-clock\"></i> "+horamensaje+"</small>"+
